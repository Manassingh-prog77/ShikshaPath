...
router.post('/Addpractise', fetchUser, async (req, res) => {
    ...
    try {
        ...
        res.status(201).json({ message: 'Quiz created successfully', practise });
    } catch (error) {
        console.error(error);
...
router.get('/fetchpractise', fetchUser, async (req, res) => {
    try {
        // Retrieve all quizzes from the database
        const questions = await Practise.find();
        ...
        res.status(200).json({ message: 'practise questions fetched successfully', questions });
    } catch (error) {
        console.error(error);
        res.status(500).json({ message: 'practise to fetch questions', error: error.message });
    }
});

// Route to delete a specific quiz by ID
router.delete('/practise/:id', fetchUser, async (req, res) => {
    ...
    try {
        ...
        const quiz = await Practise.findOneAndDelete({ _id: quizId, user: req.user.id });
